...
You are given a mock-up presence absence dataset (see green box below.) Build a model (with Lux.jl) that predicts the full species distribution for each time steps in the dataset.
...
::: {.callout-tip collapse="true"}
# Loading and processing the absence presence data
...
### Mock-up presence absence data
You'll find the mock-up presence absence data in `data/PA_data.csv`. The columns should be self explanatory. Here is the data plotted
...
You will use temperature data from CHELSA to project the deep SDM spatially. Run the following snippet. It will download the `bio1` raster from CHELSA, and load it.
