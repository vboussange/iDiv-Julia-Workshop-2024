...
![](https://files.ipbes.net/ipbes-web-prod-public-files/2024-02/EXPO%20TVDC%20045-2_CMJN_YANN%20ARTHUS%20BERTRAND.jpg)
...
$$
[\mathcal{D}({\bf n_t})]_i = \left[ \sum_j  n_{j, t} m_{j, i} - \sum_j  n_{i, t} m_{i, j}\right]
$$
...
Implementing the dispersal kernel is not trivial - you need to define it carefully to get nice performance. 

Two tricks can be used:

- 1. Defining a cut-off value for $m_{i,j}$ where above a certain distance $d_{i,j}$, $m_{i,j}$ becomes 0. 
- 2. Based on this cut-off, defining a sparse matrix $D$ so that $\mathcal{D}({\bf n}_t) = D {\bf n_t}$.

Consider using the following implementation:
...
```
Try to convince yourself that `D * u` exactly equals $\mathcal{D}({\bf n}_t)$. For more insights, check what are [Laplacian matrices](https://en.wikipedia.org/wiki/Laplacian_matrix#cite_note-10) 
:::
